 ...
 * 
 **- calculate_perpendicular_components:
...
static void calculate_perpendicular_components(t_ray ray, t_cy c, t_cyl_body *cy)
{
    cy->oc = vec_sub(ray.start, c.center);
    cy->oc_per = vec_sub(cy->oc, vec_mult(c.axis, dot_vec(cy->oc, c.axis)));
    cy->ray_dir_perp = vec_sub(ray.direction, vec_mult(c.axis, dot_vec(ray.direction, c.axis)));
}
...
float hit_cyl_body(t_ray ray, t_cy c, float r, float h)
{
    ...
    calculate_perpendicular_components(ray, c, &cy);
